...
The Phase 12 numerical primitives are instrumented with [criterion.rs](https://github.com/bheisler/criterion.rs)
so regressions in the hot math are caught at PR time. Run with:
...
Reports are HTML rendered under `target/criterion/<bench>/report/index.html`
with mean / median / slope / regression / PDF / violin plots per benchmark.
...
- **regression plot** (`regression.svg`) — OLS slope of per-iteration
  time across sample sizes; positive slope indicates asymptotic
  complexity `O(n^slope)`.
